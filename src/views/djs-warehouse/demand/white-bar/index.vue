  ...
  <div class="p-2">
    ...
    >
      <template #cell-actions="{ row }">
        <el-button v-if="canSubmit(row)" link type="primary" size="small" @click="onSubmit(row)">
          {{ t('demand.action.submit') }}
        </el-button>
        ...
        <el-button link type="info" size="small" @click="onHistory(row)">
          {{ t('demand.action.history') }}
        </el-button>
      </template>
    </BizTable>
...
<script setup name="DemandWhiteBar" lang="ts">
...
function statusActions(row: DemandManageVO): string[] {
  return allowedActions(row.demandStatus as DemandStatusCode);
}

  ...
  djs: {
    ...
    warehouse: {
      ...
      packEntry: {
        ...
        measureBelowTip: 'Packing rule {rule}kg, current weight {actual}kg. It must not be less than the rule weight — please weigh again',
        measureDeviationTitle: 'Weight above packing rule',
        measureDeviationTip: 'Packing rule {rule}kg, current weight {actual}kg (over {tolerance}% can still be submitted after confirmation)',
        measureDeviationConfirm: 'Actual {actual}kg exceeds the {rule}kg packing rule by more than {tolerance}%. Continue packing?',

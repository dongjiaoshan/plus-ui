export default {
  ...
  pig: {
    ...
    toFatten: {
      ...
      transferDateRequired: '请选择转移日期',
      operator: '转移负责人',
      operatorPlaceholder: '请选择转移负责人',
      operatorRequired: '请选择转移负责人',
      newBarn: '转移栋舍',
      newBarnPlaceholder: '请选择转移栋舍',
      newBarnRequired: '请选择转移栋舍',
      newPen: '转移栏位',
  ...
  djs: {
    ...
    warehouse: {
      ...
      packEntry: {
        ...
        vegWeightExceed: '打包重量超过领用剩余重量（{remain}g），请重新称重',
        pigAssignLabel: '猪只指定',
        noBars: '暂无待领用白条',
        agingHour: '小时',
        agingMinute: '分',
      ...
      return: {
        ...
        returnWeightRequired: '退回重量必填且 > 0',
        confirmWeightRequired: '确认重量必填且 > 0',
        dialogAddTitle: '新增退回',
        dialogEditTitle: '编辑退回',
        confirmDialogTitle: '确认退回',
        confirmStockHint: '门店→仓库方向：确认后将自动联动 stock_flow（return_in 入库）',
        confirmPlaceholderHint: '其他方向 V1 仅录入占位，不联动 stock_flow（V2 实现）',
        addSuccess: '退回已新增',
        editSuccess: '退回已编辑',
        deleteConfirm: '确认删除退回单【{no}】？',
        deleteSuccess: '退回已删除',
        confirmSuccess: '退回已确认',
        viewDetail: '查看详情',
        productKindCount: '退回品种数',
        returnWeightTotal: '退回重量',
      ...
      production: {
        ...
        column: {
          ...
          plotCode: '地块编码',
          produceTime: '生产时间',
          isDeliveryCheck: '是否已清点',
          isArrivalConfirm: '是否已到店',
          createByName: '录入人',
          remark: '备注'
        },
        dest: {
  ...
  warehouse: {
    dashboard: {
      ...
      refresh: '刷新',
      // DJS-FIX-WMS-RALN 看板对齐原型补键（今日需求/今日生产 8 项 + 退回切换 + 趋势系列 + 单位）
      kpiDemandPork: '猪肉产品',
      ...
      seriesPorkLoss: '猪肉产品损耗量',
      seriesVegLoss: '果蔬产品损耗量',
      unitHead: '头',
      unitKg: 'kg',
      unitKind: '种',
      unitPiece: '个',
  ...
  storeTrace: {
    ...
    pork: {
      ...
      materialRemain: '原材料剩余',
      materialOverWeight: '本次打包重量超过原材料「{material}」当日剩余可打包量（剩余{remain}g）',
      materialNoInbound: '门店盘点未录入原材料「{material}」的当日入库量，无法打包；请先在门店盘点录入入库量',
      pickCut: '选择追溯部位',
      opPanel: '操作',
      pigId: '猪只ID',
      pigSex: '猪只性别',
